...
public class GazetteerWaypointScreen extends Screen{
    ...
    public GazetteerWaypointScreen(Component title){
        super(title);
    }
    ...
    protected void init(){
        ...
        addButton = Button.builder(Component.translatable("buttons.add.new.waypoint"), button -> {
            ...
            GazetteerClient.addNewWaypoint((int) Math.floor(player.getX()), (int) Math.floor(player.getY()), (int) Math.floor(player.getZ()), false, false);
            refreshAll();
        ...
        addFolderButton = Button.builder(Component.literal("New Folder"), button -> {
            GazetteerWaypointColor randomColor = new GazetteerWaypointColor(new java.util.Random().nextFloat(), new java.util.Random().nextFloat(), new java.util.Random().nextFloat());
        ...
        deleteWaypointButton = Button.builder(Component.translatable("selectWorld.delete"), button -> {
            GazetteerClient.deleteWaypoint(selectedWaypointId);
        ...
        copyCoordinatesButton = Button.builder(Component.literal("---"), button -> {
            ...
            GazetteerWaypoint waypoint = GazetteerClient.variables.waypoints.get(selectedWaypointId);
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta){
        ...
        boolean hasSelection = selectedWaypointId >= 0 && selectedWaypointId < GazetteerClient.variables.waypoints.size();
    ...
    public boolean mouseClicked(@NonNull MouseButtonEvent mouseButtonEvent, boolean doubled){
        if(mouseButtonEvent.button() == 1 && copyCoordinatesButton.active && copyCoordinatesButton.isMouseOver(mouseButtonEvent.x(), mouseButtonEvent.y())){
            GazetteerWaypoint wp = GazetteerClient.variables.waypoints.get(selectedWaypointId);
            int newX, newY, newZ;
            String targetDim;
            ...
            GazetteerClient.variables.colors.add(new GazetteerWaypointColor(
                    ...
                    GazetteerClient.variables.colors.get(selectedWaypointId).getHSV()[1] / 100f,
                    GazetteerClient.variables.colors.get(selectedWaypointId).getHSV()[2] / 100f));
            GazetteerClient.variables.savedSinceLastUpdate = false;
            int newId = GazetteerClient.variables.waypoints.size() - 1;
            if(ctrlHeld){
                refreshAll();
            } else {
    ...
    private class CategoryList extends AbstractSelectionList<CategoryList.CategoryEntry>{
        ...
        public CategoryList(int left, int top, int width, int height){
            super(GazetteerWaypointScreen.this.minecraft, width, height, top, 24);
            this.listLeft = left;
    ...
    private class WaypointList extends AbstractSelectionList<WaypointList.ListEntry>{
        ...
        public WaypointList(int left, int top, int width, int height){
            super(GazetteerWaypointScreen.this.minecraft, width, height, top, 25);
            this.listLeft = left;
            this.listWidth = width;
            this.setX(left);
            refreshEntries();
        ...
        private void addFolderChildren(String parentId, int depth){
            for(GazetteerFolder folder : GazetteerClient.variables.folders){
                ...
                if(folder.expanded){
                    ...
                    for(int i = 0; i < GazetteerClient.variables.waypoints.size(); i++){
                        GazetteerWaypoint wp = GazetteerClient.variables.waypoints.get(i);
        ...
        private boolean isDescendant(GazetteerFolder ancestor, GazetteerFolder candidate){
                    ...
                    if(dropOnFolder && to >= 0 && to < children().size()){
                        ...
                        if(to < children().size()){
                            ...
                            if(toEntry instanceof FolderEntry fe){
                                ...
                                targetParentId = GazetteerClient.variables.waypoints.get(we.waypointIndex).getFolderId(selectedCategory);
                            }
                        } else if(to > 0){
                        ...
                        if(!circular){
                            ...
                            int fromIdx = GazetteerClient.variables.folders.indexOf(sourceFolderEntry.folder);
                            int toIdx;
                            if(to >= children().size()){
                                toIdx = GazetteerClient.variables.folders.size();
                            } else {
                                ListEntry toEntry = children().get(to);
                                if(toEntry instanceof FolderEntry fe){
                                    toIdx = GazetteerClient.variables.folders.indexOf(fe.folder);
                                } else {
                                    int scanIdx = -1;
                                    for(int j = to - 1; j >= 0; j--){
                                        if(children().get(j) instanceof FolderEntry fe2){
                                            scanIdx = GazetteerClient.variables.folders.indexOf(fe2.folder);
                                            break;
                                        }
                                    }
                            ...
                            if(toIdx != fromIdx && toIdx >= 0){
                                GazetteerFolder moved = GazetteerClient.variables.folders.remove(fromIdx);
                                int insertAt = toIdx > fromIdx ? toIdx - 1 : toIdx;
                                insertAt = Math.min(insertAt, GazetteerClient.variables.folders.size());
                                GazetteerClient.variables.folders.add(insertAt, moved);
                            }
                            GazetteerClient.variables.savedSinceLastUpdate = false;
                        }
                        refreshEntries();
                ...
                if(sourceEntry instanceof WaypointEntry wpEntry){
                    if(dropOnFolder && dropTargetVisualIndex >= 0 && dropTargetVisualIndex < children().size()){
                        ...
                        if(targetEntry instanceof FolderEntry folderEntry){
                            GazetteerClient.variables.waypoints.get(wpEntry.waypointIndex).setFolderId(selectedCategory, folderEntry.folder.id);
                            GazetteerClient.variables.savedSinceLastUpdate = false;
                        ...
                        if(from >= 0 && to >= 0 && from != to && from < children().size()){
                            ...
                            if(to >= children().size()){
                                ...
                                if(lastEntry instanceof WaypointEntry we){
                                    ...
                                    toGlobal = GazetteerClient.variables.waypoints.size();
                                }
                            ...
                            if(toGlobal != fromGlobal){
                                GazetteerWaypoint wp = GazetteerClient.variables.waypoints.remove(fromGlobal);
                                GazetteerWaypointColor color = GazetteerClient.variables.colors.remove(fromGlobal);
                                int insertAt = toGlobal > fromGlobal ? toGlobal - 1 : toGlobal;
                                insertAt = Math.min(insertAt, GazetteerClient.variables.waypoints.size());
                                wp.setFolderId(selectedCategory, targetFolderId);
                                GazetteerClient.variables.waypoints.add(insertAt, wp);
                                GazetteerClient.variables.colors.add(insertAt, color);
                                GazetteerClient.variables.savedSinceLastUpdate = false;
        ...
        private class FolderEntry extends ListEntry{
            ...
            public FolderEntry(GazetteerFolder folder, int depth){
                ...
                this.editBtn = new IconButton(0, 0, 12, 12, () -> {
                    ...
                    protected void extractWidgetRenderState(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta){
                        ...
                        guiGraphics.text(GazetteerVariables.minecraftClient.font, "e", getX() + 3, getY() + 2, color);
                    }
                };
                ...
                this.deleteBtn = new IconButton(0, 0, 12, 12, () -> {
                    ...
                    GazetteerClient.variables.folders.remove(folder);
                    GazetteerClient.variables.savedSinceLastUpdate = false;
                    dragSourceVisualIndex = -1;
                    dropTargetVisualIndex = -1;
                    refreshEntries();
                    ...
                    protected void extractWidgetRenderState(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta){
                        ...
                        guiGraphics.text(GazetteerVariables.minecraftClient.font, "x", getX() + 3, getY() + 2, color);
                    }
                };
                ...
                this.subfolderBtn = new IconButton(0, 0, 12, 12, () -> {
                    ...
                    GazetteerWaypointColor randomColor = new GazetteerWaypointColor(rand.nextFloat(), rand.nextFloat(), rand.nextFloat());
                    ...
                    protected void extractWidgetRenderState(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta){
                        ...
                        guiGraphics.text(GazetteerVariables.minecraftClient.font, "+", getX() + 3, getY() + 3, color);
                    }
                };
            ...
            public void extractContent(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovered, float deltaTicks){
                ...
                guiGraphics.text(GazetteerVariables.minecraftClient.font, arrow, x + 4, y + 8, folderColor);

                guiGraphics.text(GazetteerVariables.minecraftClient.font, folder.name, x + 16, y + 8, folderColor);
        ...
        private class WaypointEntry extends ListEntry{
            ...
            public void extractContent(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovered, float deltaTicks){
                ...
                int nameColor = beingDragged ? 0x80FFFFFF : GazetteerClient.variables.colors.get(waypointIndex).getHex();
                guiGraphics.text(GazetteerVariables.minecraftClient.font, waypointName.getString(), x + 25, y + 8, nameColor);
            }
