...
public class GazetteerWaypointConfig extends Screen{
    private static int id;
    private boolean renderColorPicker = false;
    private final boolean viaKeybind;
    private final GazetteerWaypoint waypoint;
    ...
    public GazetteerWaypointConfig(Component title, int waypointId, boolean viaKeybind){
        super(title);
        id = waypointId;
        this.waypoint = GazetteerClient.variables.waypoints.get(id);
        this.viaKeybind = viaKeybind;
        this.origName = waypoint.name;
        this.origX = waypoint.x;
        this.origY = waypoint.y;
        this.origZ = waypoint.z;
        this.origColorHex = GazetteerClient.variables.colors.get(id).getHexNoAlpha();
    ...
    protected void init(){
        ...
        hsv = GazetteerClient.variables.colors.get(id).getHSV();
        ...
        waypointColor.setValue(GazetteerClient.variables.colors.get(id).getHexNoAlpha());
    ...
    public static class HSVSlider extends AbstractSliderButton{
        ...
        protected int getGradientColor(){
            return GazetteerClient.variables.colors.get(id).getHex();
        }
        ...
        protected void applyValue(){
            ...
            if(!this.force){
                GazetteerClient.variables.colors.get(id).set(hsv);
                waypointColor.setValue(GazetteerClient.variables.colors.get(id).getHexNoAlpha());
                GazetteerClient.variables.savedSinceLastUpdate = false;
            }
            else{
        ...
        public void extractWidgetRenderState(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta){
            ...
            if(type == 0){
                ...
                guiGraphics.guiRenderState.addGuiElement(new GazetteerReverseColoredQuadGuiElementRenderState(RenderPipelines.GUI, TextureSetup.noTexture(), new Matrix3x2f(guiGraphics.pose()), this.getX(), this.getY(), this.getX() + this.width, this.getY() + this.height, colorStart, colorEnd, guiGraphics.scissorStack.peek()));
            }
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta){
        ...
        guiGraphics.fill(left, top, right, bottom, GazetteerClient.variables.colors.get(id).getHex());
    ...
    private void setValues(){
        ...
        if(waypointColor.isFocused()){
            GazetteerClient.variables.colors.get(id).set(waypointColor.getValue());
            hsv = GazetteerClient.variables.colors.get(id).getHSV();
    ...
    @Override
    public void onClose(){
        if(!confirmed && GazetteerConfig.escapeDiscardsChanges && id < GazetteerClient.variables.waypoints.size()){
            waypoint.name = origName;
            waypoint.x = origX;
            waypoint.y = origY;
            waypoint.z = origZ;
            GazetteerClient.variables.colors.get(id).set(origColorHex);
