...
public class GazetteerFolderConfig extends Screen{
    ...
    private GazetteerWaypointConfig.HSVSlider h, s, v;
    private static float[] hsv;
    private GazetteerWaypointColor colorObj;
    ...
    public GazetteerFolderConfig(Component title, GazetteerFolder folder, boolean isNew){
        super(title);
        this.folder = folder;
        this.isNew = isNew;
        this.colorObj = new GazetteerWaypointColor(0, 0, 0);
    ...
    protected void init(){
        ...
        this.s = new GazetteerWaypointConfig.HSVSlider((this.width - 50) / 2, (this.height - 20) / 2 + 3, 110, 15, Component.literal("S: " + hsv[1]), hsv[1] / 100, 1){
            @Override
            protected void applyValue(){
                applyValueRaw();
                hsv[getType()] = getTrueValue();
        ...
        this.v = new GazetteerWaypointConfig.HSVSlider((this.width - 50) / 2, (this.height - 20) / 2 + 20, 110, 15, Component.literal("V: " + hsv[2]), hsv[2] / 100, 2){
            @Override
            protected void applyValue(){
                applyValueRaw();
                hsv[getType()] = getTrueValue();
    ...
    @Override
    public void onClose(){
        if(!confirmed && GazetteerConfig.escapeDiscardsChanges){
            if(isNew){
                GazetteerClient.variables.folders.remove(folder);
            } else {
